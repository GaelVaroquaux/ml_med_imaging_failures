...
keys = ['lung_vs_all_medical','lung_vs_all_ai', 'breast_vs_all_medical', 'breast_vs_all_ai']

labels = ['lung, medical', 'lung, AI', 'breast, medical', 'breast, AI']

plt.figure(figsize=(3.8, 3))

plt.axvline(2016, color='.3', linestyle='--')
plt.text(2016.1, 4.9, 'Kaggle lung\nchallenge', rotation=90, va='top')


for key, label in zip(keys, label):
    plt.plot(df_counts['year'], df_counts[key])
